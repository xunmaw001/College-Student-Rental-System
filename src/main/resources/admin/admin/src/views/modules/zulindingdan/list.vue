  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
		...
		<el-form-item class="select" label="评价状态" prop="pingjiazhuangtai">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                ...
                <el-table-column
                 ...
                 :header-align="contents.tableAlign"
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.ispay!='已支付' && isAuth('zulindingdan','支付') " type="text" icon="el-icon-edit" size="small" @click="payHandler(scope.row)">支付</el-button>
                  </template>
                </el-table-column>
              <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" 
                  prop="shhf"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('zulindingdan','房屋评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="fangwupingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','pingjiazhuangtai','该房屋您已评价！','已评价,未评价'.split(',')[0])">{{ contents.tableBtnFont == 1?'房屋评价':'' }}</el-button>
                <el-button v-if="isAuth('zulindingdan','房屋评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="fangwupingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','pingjiazhuangtai','该房屋您已评价！','已评价,未评价'.split(',')[0])">{{ contents.tableBtnFont == 1?'房屋评价':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('zulindingdan','房屋评价') && contents.tableBtnIcon == 0" type="success" size="mini" @click="fangwupingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','pingjiazhuangtai','该房屋您已评价！','已评价,未评价'.split(',')[0])">{{ contents.tableBtnFont == 1?'房屋评价':'' }}</el-button>
...
export default {
  ...
  methods: {
    ...
    fangwupingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.fangwupingjiaCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
    ...
    payHandler(row){
      this.$storage.set('paytable','zulindingdan');
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          fangwumianji: row.fangwumianji,
          fangwuzujin: row.fangwuzujin,
          zulinyueshu: row.zulinyueshu,
          dingdanjine: row.dingdanjine,
          dingdanbeizhu: row.dingdanbeizhu,
          fangzhuzhanghao: row.fangzhuzhanghao,
          pingjiazhuangtai: row.pingjiazhuangtai,
