<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="定金金额" prop="dingjinjine">
          <el-input v-model="ruleForm.dingjinjine" 
              placeholder="定金金额" clearable  :readonly="ro.dingjinjine"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="定金金额" prop="dingjinjine">
              <el-input v-model="ruleForm.dingjinjine" 
                placeholder="定金金额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="租赁状态" prop="zulinzhuangtai">
        ...
        <div v-else>
          <el-form-item class="input" label="租赁状态" prop="zulinzhuangtai">
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fangwutupian = true;
          ...
          if(o=='dingjinjine'){
            this.ruleForm.dingjinjine = obj[o];
	    this.ro.dingjinjine = true;
            continue;
          }
          if(o=='xiadanriqi'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.zulinzhuangtaiOptions = "已下单,未下单".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `dingjinliufang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dingjinliufang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dingjinliufangCrossAddOrUpdateFlag = false;
						 this.parent.search();
